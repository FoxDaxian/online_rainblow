	...
	export default {
		...
		methods:{
			...
			upFn(index,id,e){
				...
				ev.stopPropagation();
				this.$http({
					url:"http://rbblog.space/blog/home/index/toUp",
					method:"post",
					body:{
						id:id
					},
			...
			downFn(index,id,e){
				...
				this.$http({
					url:"http://rbblog.space/blog/home/index/toDown",
					method:"post",
					body:{
						id:id
					},
			...
			sendComment(id,parent_id = 0){
				...
				if( this.$store.state.userInfo === null ){
					...
					if( formatText.trim().length === 0 && !/img/.test(html) ){
						...
						//带有html的文本 -->存储到数据库的
						this.$http({
							url:"http://rbblog.space/blog/home/index/send_comment",
							method:"post",
							body:{
								//发送到数据库的评论
								parent_id:parent_id,
		...
		mounted(){
			...
			let getComment_fn = () => {
				this.$http({
					url:"http://rbblog.space/blog/home/index/getComment",
					method:"post",
					body:{
						artical_id:this.detail_data.id
					},
